...
#| label: wd-4-string
# Lab 5
# Revised: used a stringr function to create the first name variable

witness2 <- person |>
  mutate( first_name = str_remove(name, " .*" )) |>
...
#| label: wd-6-anti
# Lab 5
# Revision: switched to anti_join
# Use: This anti-join can be useful to exclude people who did give interviews to find those with no interviews

suspects_with_no_interview <- facebook_event_checkin_suspect |>
  anti_join( y = interview,
             by = join_by( person_id == person_id )
            )
# With no interviews by Miranda, she is the final suspect
```
...
median_region_income <- ca_childcare |>
  ...
  summarize(median_household_income = median(mhi_2018, na.rm =TRUE)) |>
  ungroup() |>
  pivot_wider(
    names_from = study_year,
    values_from = median_household_income,
    names_prefix = "median_household_income_year_"
  ) |>
  arrange(desc(median_household_income_year_2018))
```
...
rescale_01 <- function(vec) {
  ...
  if (length(vec) <= 1) stop("Input vector must have a length greater than 1")
  min_max <- range(vec, na.rm = TRUE)
  (vec - min_max[1]) / (min_max[2] - min_max[1])
...
#| label: dvs-2-ex-2
# Lab 2 Question 16
# Revision: Applied a colorblind friendly pallete and color = gray. Gray is one of the most color-blind friendly colors for a plot.
...
#| label: dvs-2-ex-5
# Lab 4 Question 7
# Revised: now includes fct_reorder2()
...
ggplot(data = surveys,
  ...
  geom_text(aes(x = 60, y = 1),
           label = "Albigula species has the largest weight distribution",
           color = "orangered",
           fontface = "bold",
           size = 3,
           hjust = 0,
           vjust = -8) +
  geom_segment(aes(x = 100, y = 1.5, xend = 105, yend = 3.5),
...
#| label: dvs-4-across
# Lab 7 Question 1
#Revised: use across wuth sum to count missing values

fish |>
  summarize(
    n_obs_missing_values = sum(!complete.cases(fish)),
    across(everything(), ~ sum(is.na(.)), .names = "{.col}_has_missing_value")
)
```
...
evals |>
  ...
  pivot_longer(cols = everything(), 
               names_to = "Demographic", values_to = "Group") |>
...
pivot_table(penguins, row = species, col = sex)
```
...
fish |>
  ...
  kbl(col.names = c("Variable", "Number of Missing Values")) |>
...
#| label: pe-2-2
# Lab 8 Question 2
# Revision: Added the input check for data frame
...
#| label: pe-2-3
# Lab 8 Question 1
# Revised: Added named arguments, added data frame input check, and use ~ & .x in rescale_01

rescale_01 <- ~ {
  if(!is.numeric(.x)) stop("Input must be numeric")
  if (length(.x) <= 1) stop("Input vector must have a length greater than 1")
...
condition_function <- function(weight, length) {
  if(!is.numeric(weight) | !is.numeric(length)) stop("Weight and Length must be numeric")
  (weight / length^3) * 100
}

fish <- fish |>
  mutate(condition_index = condition_function(weight, length))
```
...
#| label: pe-4-2
# Lab 4 Question 7
# Revised: Connected into one pipeline
ca_childcare_2 <- ca_childcare |>
  select(
    county_name, 
    region, 
...
mycifun <- function(beta0, beta1, n) {
  ...
  make_data_frame = tibble(x, y)
  fit_linear_model = lm(y ~ x, data = make_data_frame)
  check_population_slope = tidy(fit_linear_model, conf.int = TRUE) |>
    filter(term == "x")
  true_population_slope = beta1
  check_population_slope <- check_population_slope |>
    mutate(
      cover = if_else(true_population_slope >= conf.low & 
                      true_population_slope <= conf.high,
                      1, 0)
    )

  check_population_slope |>
    select(
